# Salut, Bienvenue 👋🏾

[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering) [![Linkedin Badge](https://img.shields.io/badge/-alexander_oweka-blue?style=for-the-badge&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/alexander-oweka-6bb086166)](https://www.linkedin.com/in/alexander-oweka-6bb086166/) [![Twitter Badge](https://img.shields.io/badge/-@OwekaAlexander-1ca0f1?style=for-the-badge&logo=twitter&logoColor=white&link=https://twitter.com/OwekaAlexander)](https://twitter.com/OwekaAlexander)

I'm a Software Engineer, currently working as a Frontend Developer at [Prunedge Technologies](https://prunedge.com). I'm passionate about building neat & responsive UI, anime, and video/mobile games.

**Here's a quick summary about me**:

- 😊 Pronouns: He/him
- 💡 Fun fact: I'm a student of AltSchool Africa [School of Software Engineering](https://altschoolafrica.com/schools/engineering) Class of 2022.
- 🌱 I’m currently learning TypeScript, PHP and mySQL.
- 🎧 I’m open to opportunities in open source projects, hackathons, internships, and junior-level roles.
- 💼 Job interests: Frontend Engineer, Web Developer, or UI Engineer (Intern or Junior level).
- 🛠 Tech stack: HTML, CSS, SCSS, JavaScript, TypeScript, Material UI, PHP, WordPress
- 📫 You can [view my resume](#) and contact me by emailing alexanderolalekanoweka@gmail.com.
- 👨🏾‍💻 Check out my live site: [Al X](alexander-oweka.netlify.app).

---

| <img align="center" src="https://github-readme-stats.vercel.app/api?username=AlexOla-NG&show_icons=true&include_all_commits=true&hide_border=true" alt="Alex's GitHub stats" /> | <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=AlexOla-NG&langs_count=8&layout=compact&hide_border=true" alt="Alex's GitHub stats" /> |
| ------------- | ------------- |

<!---
AlexOla-NG/AlexOla-NG is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
